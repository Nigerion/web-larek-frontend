<!--# Интернет-магазин для заказа дополнительных скилов.-->
# Интернет-магазин.
## О проекте
...
имеет общую разметку модального окна, а так же методы :<br>
...
class Model <br>
Базовая модель<br>
- `emitChanges` -используется для того, чтобы cообщить всем что модель поменялась<br>

class Card<br>
- `getPrice` - используется для получения прайса<br>
...
```
## Статус проекта
На данный момент не планируется никаких доработок проекта.
